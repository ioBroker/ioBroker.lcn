...
class CMDS {
            ...
            return [
                {
                    segment,
                    module,
                    error: CMDS.ACK_ERRORS[error] || error,
                },
            ];
        }
    }
     ...
     *
     * @param {object} address address info {segment, module, isGroup}
     * @param {number} motor motor number 1-4
     * @param {string} position 0-100
     *
     */
     ...
     * @param {string} command 1- short, 2- long, 0- release
     *
     */
     ...
     *
     */
    static buildGetSerial(address, type, part) {
    ...
    static buildGetSegmentRepeater(address, isAllSegments, groupID) {
        ...
        return CMDS.buildAddress(
            address,
            false,
            'G' + (isAllSegments ? '003' : '000') + CMDS.formatNumber(groupID) + '.SK',
        );
    }
